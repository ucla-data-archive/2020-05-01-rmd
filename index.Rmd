...
**Read in data** - We'll use the `gapminder` package. We can use the `data()` function to get R to read data into our environment, so it shows up in our environment.
...
```{r see-levels, results='hide'}
levels(gapminder$country)
```

### Pick Four Countries

Now pick four countries that you are intrested in.  Replace with the countries name below.
